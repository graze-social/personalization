...
use crate::redis::{Keys, RedisClient, DEFAULT_RETENTION_DAYS, retention_dates, ttl_for_date};
...
impl UriInterner {
    ...
    pub async fn get_uri(&self, post_id: &str) -> Result<Option<String>> {
        ...
        if let Some(date) = intern_date_from_post_id(post_id) {
            if let Some(uri) = self.redis.hget(&Keys::id_to_uri_date(date), post_id).await? {
    ...
    pub async fn get_uris_batch(&self, post_ids: &[String]) -> Result<HashMap<String, String>> {
        ...
        for id in &to_fetch {
            if let Some(date) = intern_date_from_post_id(id) {
                by_date.entry(date.to_string()).or_default().push(id.clone());
